...
// Column indices for the file list store
enum {
  COL_ICON = 0,
  COL_NAME,
  COL_TYPE,
...
static GtkWidget *build_file_list(AppState *state) {
  // List store: icon name (string), display name, type label
  ...
  gtk_tree_view_column_add_attribute(name_col, icon_renderer,
                                     "icon-name", COL_ICON);
  gtk_tree_view_column_pack_start(name_col, name_renderer, TRUE);
  gtk_tree_view_column_add_attribute(name_col, name_renderer,
                                     "text", COL_NAME);
...
GtkWidget *create_main_window() {
  ...
  // Store state pointer on the window for convenience
